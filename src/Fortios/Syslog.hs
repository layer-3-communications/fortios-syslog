...
afterEquals !b !b0 = case fromIntegral @Int @Word len of
  ...
  6 -> case G.hashString6 arr off of
    ...
    G.H_action -> case zequal6 arr off 'a' 'c' 't' 'i' 'o' 'n' of
      0# -> do
        ...
        P.effect (Builder.push atom b0)
      _ -> discardUnknownField b0
    G.H_cfgobj -> case zequal6 arr off 'c' 'f' 'g' 'o' 'b' 'j' of
      0# -> do
        val <- asciiTextField InvalidCfgObj
        let !atom = CfgObj val
        P.effect (Builder.push atom b0)
      _ -> discardUnknownField b0
    ...
    G.H_method -> case zequal6 arr off 'm' 'e' 't' 'h' 'o' 'd' of
      0# -> do
        ...
        P.effect (Builder.push atom b0)
      _ -> discardUnknownField b0
    G.H_status -> case zequal6 arr off 's' 't' 'a' 't' 'u' 's' of
      0# -> do
        val <- asciiTextField InvalidStatus
        let !atom = Status val
        P.effect (Builder.push atom b0)
      _ -> discardUnknownField b0
  ...
  7 -> case G.hashString7 arr off of
    G.H_cfgpath -> case zequal7 arr off 'c' 'f' 'g' 'p' 'a' 't' 'h' of
      0# -> do
        val <- asciiTextField InvalidCfgPath
        let !atom = CfgPath val
        P.effect (Builder.push atom b0)
      _ -> discardUnknownField b0
